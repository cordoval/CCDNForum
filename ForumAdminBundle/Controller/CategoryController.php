...
namespace CCDNForum\ForumAdminBundle\Controller;
...
class CategoryController extends ContainerAware
{
	...
	public function createAction()
	{
		...
		{	
			$this->container->get('session')->setFlash('notice', 
				$this->container->get('translator')->trans('flash.category.create.success', array(), 'CCDNForumForumAdminBundle'));
		...
		else
		{
			...
			$crumb_trail = $this->container->get('crumb_trail')
				->add($this->container->get('translator')->trans('crumbs.category.index', array(), 'CCDNForumForumAdminBundle'), 
					$this->container->get('router')->generate('cc_admin_forum_category_index'), "home")
				->add($this->container->get('translator')->trans('crumbs.category.create', array(), 'CCDNForumForumAdminBundle'), 
					$this->container->get('router')->generate('cc_admin_forum_category_create'), "edit");
					
			return $this->container->get('templating')->renderResponse('CCDNForumForumAdminBundle:Category:create.html.' . $this->getEngine(), array(
				'crumbs' => $crumb_trail,
	...
	public function editAction($category_id)
	{
		...
		{
			$this->container->get('session')->setFlash('notice', 
				$this->container->get('translator')->trans('flash.category.edit.success', array(), 'CCDNForumForumAdminBundle'));
		...
		else
		{
			...
			$crumb_trail = $this->container->get('crumb_trail')
				->add($this->container->get('translator')->trans('crumbs.category.index', array(), 'CCDNForumForumAdminBundle'), 
					$this->container->get('router')->generate('cc_admin_forum_category_index'), "home")
				->add($this->container->get('translator')->trans('crumbs.category.edit', array('%category_name%' => $category->getName()), 'CCDNForumForumAdminBundle'), 
					$this->container->get('router')->generate('cc_admin_forum_category_edit', array('category_id' => $category_id)), "edit");
					
			return $this->container->get('templating')->renderResponse('CCDNForumForumAdminBundle:Category:edit.html.' . $this->getEngine(), array(
				'category' => $category,
				'crumbs' => $crumb_trail,
				'form' => $form->createView(),
			));
	...
	public function deleteAction($category_id)
	{
		...
		$crumb_trail = $this->container->get('crumb_trail')
			->add($this->container->get('translator')->trans('crumbs.category.index', array(), 'CCDNForumForumAdminBundle'), 
				$this->container->get('router')->generate('cc_admin_forum_category_index'), "home")
			->add($this->container->get('translator')->trans('crumbs.category.delete', array('%category_name%' => $category->getName()), 'CCDNForumForumAdminBundle'),
				$this->container->get('router')->generate('cc_admin_forum_category_delete', array('category_id' => $category->getId())), "trash");
		
		return $this->container->get('templating')->renderResponse('CCDNForumForumAdminBundle:Category:delete_category.html.' . $this->getEngine(), array(
			'category' => $category,
			'crumbs' => $crumb_trail,
		));
	}
	...
	public function deleteConfirmedAction($category_id)
	{
		...
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.category.delete.success', array(), 'CCDNForumForumAdminBundle'));
	...
	public function reorderAction($category_id, $direction)
	{
		...
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.category.reorder.success', array(), 'CCDNForumForumAdminBundle'));
