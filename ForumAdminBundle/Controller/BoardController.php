...
class BoardController extends ContainerAware
{
	...
	public function createAction($category_id)
	{
		...
		if ($formHandler->process())	
		{
			$this->container->get('session')->setFlash('notice', 
				$this->container->get('translator')->trans('flash.board.create.success', array(), 'CCDNForumForumAdminBundle'));
		...
		else
		{
			...
			$crumb_trail = $this->container->get('crumb_trail')
				...
				->add($this->container->get('translator')->trans('crumbs.board.create', array(), 'CCDNForumForumAdminBundle'), 
					$this->container->get('router')->generate('cc_admin_forum_board_create'), "edit");
					
			return $this->container->get('templating')->renderResponse('CCDNForumForumAdminBundle:Board:create.html.' . $this->getEngine(), array(
				'crumbs' => $crumb_trail,
	...
	public function editAction($board_id)
	{
		...
		{	
			$this->container->get('session')->setFlash('notice', 
				$this->container->get('translator')->trans('flash.board.edit.success', array(), 'CCDNForumForumAdminBundle'));
		...
		else
		{
			...
			$crumb_trail = $this->container->get('crumb_trail')
				->add($this->container->get('translator')->trans('crumbs.category.index', array(), 'CCDNForumForumAdminBundle'), 
					$this->container->get('router')->generate('cc_admin_forum_category_index'), "home")
				->add($this->container->get('translator')->trans('crumbs.board.edit', array('%board_name%' => $board->getName()), 'CCDNForumForumAdminBundle'), 
					$this->container->get('router')->generate('cc_admin_forum_board_edit', array('board_id' => $board_id)), "edit");
					
			return $this->container->get('templating')->renderResponse('CCDNForumForumAdminBundle:Board:edit.html.' . $this->getEngine(), array(
				'board' => $board,
				'crumbs' => $crumb_trail,
				'form' => $form->createView(),
			));
	...
	public function deleteAction($board_id)
	{
		...
		$crumb_trail = $this->container->get('crumb_trail')
			->add($this->container->get('translator')->trans('crumbs.category.index', array(), 'CCDNForumForumAdminBundle'), 
				$this->container->get('router')->generate('cc_admin_forum_category_index'), "home")
			->add($this->container->get('translator')->trans('crumbs.board.delete', array('%board_name%' => $board->getName()), 'CCDNForumForumAdminBundle'), 
				$this->container->get('router')->generate('cc_admin_forum_board_delete', array('board_id' => $board->getId())), "trash");
		
		return $this->container->get('templating')->renderResponse('CCDNForumForumAdminBundle:Board:delete_board.html.' . $this->getEngine(), array(
			'board' => $board,
			'crumbs' => $crumb_trail,
		));	
	}
	...
	public function deleteConfirmedAction($board_id)
	{
		...
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.board.delete.success', array(), 'CCDNForumForumAdminBundle'));
...
//		$em = $this->container->get('doctrine.orm.entity_manager');
		...
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.board.reorder.success', array(), 'CCDNForumForumAdminBundle'));
