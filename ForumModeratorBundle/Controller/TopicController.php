...
/*
 * This file is part of the CCDN ForumModeratorBundle
 *
...
namespace CCDNForum\ForumModeratorBundle\Controller;
...
class TopicController extends ContainerAware
{
	...
	public function showClosedAction($page)
	{
		...
		$crumb_trail = $this->container->get('crumb_trail')
			->add($this->container->get('translator')->trans('crumbs.topic.closed.index', array(), 'CCDNForumForumModeratorBundle'), 
	...
	public function closeAction($topic_id)
	{
		...
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.topic.close.success', array('%topic_title%' => $topic->getTitle()), 'CCDNForumForumModeratorBundle'));
	...
	public function reopenAction($topic_id)
	{
		...
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.topic.reopen.success', array('%topic_title%' => $topic->getTitle()), 'CCDNForumForumModeratorBundle'));
	...
	public function moveAction($topic_id)
	{
		...
		{
			$this->container->get('session')->setFlash('notice', 
				$this->container->get('translator')->trans('flash.topic.move.success', array('%topic_title%' => $topic->getTitle()), 'CCDNForumForumModeratorBundle'));
		...
		else
		{
			...
			$crumb_trail = $this->container->get('crumb_trail')
				...
				->add($this->container->get('translator')->trans('crumbs.topic.change_board', array(), 'CCDNForumForumModeratorBundle'), 
					$this->container->get('router')->generate('cc_moderator_forum_topic_change_board', array('topic_id' => $topic->getId())), "edit");

			$form = $formHandler->getForm();
			
			return $this->container->get('templating')->renderResponse('CCDNForumForumModeratorBundle:Topic:change_board.html.' . $this->getEngine(), array(
				'topic' => $topic,
				'crumbs' => $crumb_trail,
	...
	public function restoreAction($topic_id)
	{
		...
		// set flash message
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.topic.restore.success', array('%topic_title%' => $topic->getTitle()), 'CCDNForumForumModeratorBundle'));
...
//			'confirmation_message' => $confirmationMessage,
	...
	public function deleteConfirmedAction($topic_id)
	{
		...
		// set flash message
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.topic.delete.success', array('%topic_title%' => $topic->getTitle()), 'CCDNForumForumModeratorBundle'));
