...
class FlagController extends ContainerAware
{
	...
	public function showFlaggedAction($page, $status)
	{
		...
		$crumb_trail = $this->container->get('crumb_trail')
			->add($this->container->get('translator')->trans('crumbs.flag.index', array(), 'CCDNForumForumModeratorBundle'), 
	...
	{	
		...
		$crumb_trail = $this->container->get('crumb_trail')
			->add($this->container->get('translator')->trans('crumbs.flag.index', array(), 'CCDNForumForumModeratorBundle'), 
				$this->container->get('router')->generate('cc_moderator_forum_show_all_flagged_posts'), "home")
			->add($this->container->get('translator')->trans('crumbs.flag.show', array('%flag_id%' => '#' . $flag->getId()), 'CCDNForumForumModeratorBundle'),
	...
	{		
		...
		if ($formHandler->process())
		{
			$this->container->get('session')->setFlash('notice', 
				$this->container->get('translator')->trans('flash.flag.update.success', array('%flag_id%' => $flag_id), 'CCDNForumForumModeratorBundle'));
		...
		else
		{
			...
			$crumb_trail = $this->container->get('crumb_trail')
				...
				->add($this->container->get('translator')->trans('crumbs.flag.show', array('%flag_id%' => '#' . $flag->getId()), 'CCDNForumForumModeratorBundle'),
					$this->container->get('router')->generate('cc_moderator_forum_show_flag', array('flag_id' => $flag->getId())), "flag")
				->add($this->container->get('translator')->trans('crumbs.flag.mark', array(), 'CCDNForumForumModeratorBundle'),
