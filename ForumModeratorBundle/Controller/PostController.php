...
class PostController extends ContainerAware
{
	...
	public function showLockedAction($page)
	{
		...
		$crumb_trail = $this->container->get('crumb_trail')
			->add($this->container->get('translator')->trans('crumbs.post.locked.index', array(), 'CCDNForumForumModeratorBundle'), 
	...
	public function lockAction($post_id)
	{
		...
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.post.lock.success', array('%post_id%' => $post_id), 'CCDNForumForumModeratorBundle'));
	...
	public function unlockAction($post_id)
	{
		...
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.post.unlock.success', array('%post_id%' => $post_id), 'CCDNForumForumModeratorBundle'));
	...
	public function restoreAction($post_id)
	{
		...
		// set flash message
		$this->container->get('session')->setFlash('notice', 
			$this->container->get('translator')->trans('flash.post.restore.success', array('%post_id%' => $post_id), 'CCDNForumForumModeratorBundle'));
